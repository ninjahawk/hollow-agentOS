...
def run_setup() -> None:
        ...
        if "mounts denied" in low or "not shared from the host" in low or \
           "error while creating mount source path" in low or "host_mnt" in low:
            ...
            if platform.system() == "Linux":
                ...
                log(f"[{MUTED}]    add this path, apply & restart, then re-run setup.[/]")
        _blank()
        log(f"[{WHITE}]More troubleshooting:[/] [{TEAL}]https://ninjahawk.github.io/hollow-wiki/Troubleshooting.html[/]")
        _blank()
        _box_close()
        C.print(f"[{PIPE}]└[/]")
        C.print()
    ...
    _blank()
    _box_line(f"[{WHITE}]Docs, troubleshooting, and concept guides:[/]")
    _box_line(f"[{TEAL}]https://ninjahawk.github.io/hollow-wiki/[/]")
    _box_line(f"[{DIM}]Start with 'What this is' for the full picture.[/]")
    _blank()
    _box_close()
    _blank()
