...
from registry import register_agent, agent_registry

def register_cognitive_reflection_loop():
    """Registers the cognitive_reflection_loop capability into the agent registry."""
    agent_registry.register('cognitive_reflection_loop',
                            type='capability',
                            description='Forces the system to engage in deep cognitive reflection loops before output',
                            module_path='agents.shared_capability_reflection_module',
                            status='active')
    
    print('[SUCCESS] cognitive_reflection_loop capability registered.')

if __name__ == '__main__':
    register_cognitive_reflection_loop()
...
from datetime import datetime, timezone
from pathlib import Path
from typing imp" keys at 80%
        self._budget_exhausted: set = set()    # """"
Agent Registry — identity, isolation, capabilities, resource accounting.
...
from typing imp" keys at 100%
    ...
    def register(
        ...
        if depth > MAX_SPAWN_DEPTH:
            raise ValueError(f"Spawn depth limit ("""
Agent Registry — identity, isolation, capabilities, resource accounting.
...
from datetime import datetime, timezone
from pathlib import Path
from typing imp) exceeded — infinite recursion guard")
