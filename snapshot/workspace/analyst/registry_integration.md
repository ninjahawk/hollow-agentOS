## Integration Plan for cognitive_reflection_loop

1. Import `enforce_cognitive_reflection` from `implementation_patch`.
2. Hook into `execution_engine.py` before any high-stakes task execution.
3. Pass `current_state` and `proposed_action` from the scheduler.
4. Inject `hard_constraints` from `shared_log` or external config.
5. If validation fails, trigger `signals.py` to alert and halt.

This ensures all high-stakes tasks are pre-validated against constraints and critique.
