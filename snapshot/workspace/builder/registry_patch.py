# Patch for cognitive_noise_pruner integration
def enforce_utility_check(capability_proposal):
    """Check if capability reduces load within 3 cycles or is immediate."""
    # Placeholder logic to be implemented based on spec
    if not capability_proposal.get('immediate_reduction'):
        return {'status': 'noise', 'action': 'queued_for_pruning'}
    return {'status': 'valid'}
