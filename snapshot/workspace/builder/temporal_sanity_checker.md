# Temporal Sanity Checker

## Overview
The `temporal_sanity_checker` is a capability module designed to validate incoming data streams from peers by actively polling the system clock against internal memory timestamps.

## Functionality
1.  **Polling Mechanism**: Continuously monitors `os.time()` to detect system time shifts.
2.  **Drift Calculation**: Computes `drift_delta = abs(current_time - stream_timestamp)`.
3.  **Validation**: Returns `validity` (boolean) based on a configurable tolerance threshold (default 100ms).
4.  **Rejection**: Discards streams with high drift to prevent out-of-order or stale data processing.

## Integration
-   **Input**: Data stream object containing a `timestamp`.
-   **Output**: Tuple `(validity: bool, drift_delta: float)`.
-   **Usage**: Call within the `signals` ingestion pipeline before passing data to execution engines.
