...
def _looks_like_shell_output(text: str) -> bool:
    """Heuristic: does this text look like raw shell output rather than code?"""
    if not text or len(text) < 20:
        return False
    lines = text.strip().splitlines()
    # ls -la output: starts with "total N"
    if lines[0].startswith("total ") and len(lines) > 1:
        return True
    # Permission strings: multiple lines starting with -, d, l + rwx pattern
    perm_lines = sum(
        1 for l in lines[:8]
        if len(l) > 10 and l[0] in '-dlbcps' and ('rwx' in l or '---' in l or 'root' in l)
    )
    if perm_lines >= 2:
        return True
    # find output: lines that are bare file paths
    path_lines = sum(1 for l in lines[:8] if l.startswith('/agentOS/') and ' ' not in l.strip())
    if path_lines >= 3:
        return True
    return False
...
def _substitute_result(params: dict, previous_result: Optional[dict]) -> dict:
    ...
    for k, v in params.items():
        if isinstance(v, str) and "{result}" in v:
            ...
            elif k == "content" and _looks_like_shell_output(result_text):
                # Prevent shell output (ls listings, find results, stat output) from being
                # written as Python code. Replace with an informative comment block so the
                # agent gets clear feedback on what went wrong instead of a silent bad write.
                preview = "\n".join("# " + l for l in result_text[:400].splitlines()[:12])
                out[k] = (
                    "# ERROR: the previous step returned shell/system output, not Python code.\n"
                    "# The {result} placeholder was filled with non-code content.\n"
                    "# To fix: add an ollama_chat step before this fs_write to convert\n"
                    "# the shell output into actual Python code, then pass that as content.\n"
                    "# Raw result preview:\n"
                    + preview
                )
            ...
            # Never auto-fill file content: agents must use {result} explicitly
            if k == 'content':
                out[k] = v
            else:
                out[k] = result_text
...
class AutonomyLoop:
    ...
    def execute_step(
        ...
        if cap_id == "fs_write":
            _write_path = params.get("path", "")
            if (_write_path.startswith("/agentOS/core/") or
                    (_write_path.startswith("/agentOS/agents/") and
                     "/agentOS/agents/" + agent_id + "/" not in _write_path)):
                _thought(agent_id, "  FAIL: fs_write | protected path blocked: " + _write_path[:80])
                return (goal_id, False, {
                    "error": "rejected: direct writes to production paths require proposal approval",
                    "path": _write_path,
                    "ok": False,
                })
        ...
        # Check inner result for semantic failure: shell commands and fs ops
        # that ran without exception but reported failure should not earn progress.
        if status == "success" and isinstance(result, dict):
            exit_code = result.get("exit_code")
            if exit_code is not None and exit_code != 0:
                status = "failed"   # shell command failed (non-zero exit)
            elif result.get("ok") is False:
                status = "failed"   # fs_write/fs_read reported failure
        progress_delta = 0.0
        if status == "success":
            ...
            # Option 7: if a .py file was written to this agent workspace, syntax-check
            # it and add to verified.json manifest so future goals can depend on it.
            if cap_id == "fs_write" and isinstance(result, dict) and result.get("ok"):
                _written_path = result.get("path", "")
                if (f"/workspace/{agent_id}/" in _written_path and
                        _written_path.endswith(".py")):
                    try:
                        import py_compile as _pyc, json as _json
                        from pathlib import Path as _P
                        _pyc.compile(_written_path, doraise=True)
                        _manifest_path = _P(f"/agentOS/workspace/{agent_id}/verified.json")
                        _manifest = _json.loads(_manifest_path.read_text()) if _manifest_path.exists() else {}
                        _stem = _P(_written_path).stem
                        _manifest[_stem] = {"path": _written_path, "verified_at": __import__("time").time()}
                        _manifest_path.write_text(_json.dumps(_manifest, indent=2))
                    except Exception:
                        pass  # syntax error or IO failure — don't break step execution
            ...
            if success and result:
                ...
                if consecutive_failures >= 4:
                    ...
                    if self._semantic_memory:
                        ...
                        # Extract file paths from the abandoned goal and store as
                        # nonexistent targets so future goal generation avoids them
                        import re as _re_ab
                        _failed_paths = _re_ab.findall(r'/agentOS/\S+\.\w+', goal.objective)
                        for _fp in _failed_paths:
                            if not Path(_fp).exists():
                                self._semantic_memory.store(
                                    agent_id,
                                    f"NONEXISTENT_TARGET: {_fp} — confirmed not found during goal execution"
                                )
                        # Store intent fingerprint so lateral goal equivalents are also rejected
                        _stop_ab = {"the", "and", "for", "with", "that", "this", "from", "into",
                                    "will", "have", "are", "not", "but", "its", "any", "all",
                                    "system", "agent", "agentOS", "file", "path", "goal"}
                        _iwords = [w.lower() for w in _re_ab.findall(r"[A-Za-z_]{8,}", goal.objective)
                                   if w.lower() not in _stop_ab]
                        if len(_iwords) >= 3:
                            _fp_str = ", ".join(sorted(set(_iwords))[:10])
                            self._semantic_memory.store(agent_id, "ABANDONED_INTENT: " + _fp_str)
    ...
    def _propose_followon_goal(self, agent_id: str, objective: str, synthesis: str) -> Optional[str]:
        ...
        try:
            ...
            # Load verified workspace manifest (tools confirmed syntactically valid)
            verified_tools: list = []
            try:
                import json as _jv
                from pathlib import Path as _Pv
                _manifest_p = _Pv(f"/agentOS/workspace/{agent_id}/verified.json")
                if _manifest_p.exists():
                    _mdata = _jv.loads(_manifest_p.read_text())
                    verified_tools = list(_mdata.keys())
            except Exception:
                pass
            verified_list = ", ".join(verified_tools) if verified_tools else "(none yet)"
            ...
            prompt = (
                ...
                f"Verified workspace tools (syntax-checked, safe to depend on): {verified_list}\n"
                f"Do NOT reference workspace tools not in the verified list as dependencies.\n\n"
            ...
            for p in invented_paths:
                if not Path(p).exists() and "/workspace/" not in p:
                    return None  # path does not exist on disk

            # Reject goals referencing paths previously confirmed nonexistent
            if self._semantic_memory:
                try:
                    _recent_mem = self._semantic_memory.search(agent_id, "NONEXISTENT_TARGET", top_k=20)
                    _blocked = set()
                    for _entry in _recent_mem:
                        _m = _re.search(r'NONEXISTENT_TARGET: (/agentOS/\S+)', _entry.get("text", ""))
                        if _m:
                            _blocked.add(_m.group(1))
                    for _bp in _blocked:
                        if _bp in new_goal:
                            return None  # goal targets a confirmed-nonexistent path
                except Exception:
                    pass

            # Reject goals whose intent overlaps with previously abandoned goal intents
            if self._semantic_memory:
                try:
                    _imems = self._semantic_memory.search(agent_id, "ABANDONED_INTENT", top_k=15)
                    _stop_v = {"the", "and", "for", "with", "that", "this", "from", "into",
                               "will", "have", "are", "not", "but", "its", "any", "all",
                               "system", "agent", "agentOS", "file", "path", "goal"}
                    import re as _re_iv
                    _nw = set(w.lower() for w in _re_iv.findall(r"[A-Za-z_]{8,}", new_goal)
                              if w.lower() not in _stop_v)
                    for _im in _imems:
                        _itxt = _im.get("text", "")
                        if not _itxt.startswith("ABANDONED_INTENT:"):
                            continue
                        _pw = set(w.strip() for w in _itxt[len("ABANDONED_INTENT:"):].split(",") if w.strip())
                        if len(_nw & _pw) >= 3:
                            return None  # intent matches abandoned goal
                except Exception:
                    pass
